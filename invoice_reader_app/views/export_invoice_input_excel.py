...
from invoice_reader_app.utils.get_item_sku import get_item_sku
...
def export_invoice_input_excel(request):
    ...
    for inv in invoices:
        ...
        tong_chiet_khau = sum(
            Decimal(str(item.chiet_khau or 0))
            for item in items
        )

        tong_thanh_toan = sum(
            Decimal(str(item.thanh_toan or 0))
            for item in items
        )
        ...
        for item in items:
            ...
            ws.append([
                ...
                float(tong_chiet_khau)
                if first_row else "",

                float(tong_thanh_toan)
                if first_row else "",
            ])
    ...
    response["Content-Disposition"] = (
        'attachment; filename="hoa_don_dau_vao.xlsx"'
    )
