...
from django.utils.dateparse import parse_date
from decimal import Decimal
from invoice_reader_app.models_purchaseorder import CashReceipt
...
def invoice_export_list(request):
    ...
    invoices = invoices.annotate(
        total_paid_amount=F("bank_paid") + F("cash_paid")
    )

    # ================= HAS EXPORTED =================
    # Chỉ cần Invoice đã có PurchaseOrder liên kết
    # thì được xem là đã tạo PX.
    invoices = invoices.annotate(
        has_exported=F("da_tao_px")
    )
    ...
    # ================= ORDER PX =================
    # Chưa tạo PX lên trước
    # Đã tạo PX xuống dưới
