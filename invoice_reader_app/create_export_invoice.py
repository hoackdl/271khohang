...
from invoice_reader_app.models_purchaseorder import PurchaseOrder, PurchaseOrderItem
...
def export_invoice_waiting_list(request):
    ...
    if q:
        invoices = invoices.filter(
            Q(ten_nguoi_mua__icontains=q) |
            Q(ma_so_thue_mua__icontains=q)
        )
...
def create_export_invoice(request):
    ...
    max_invoice_date = (
        max_invoice.ngay_hd.strftime("%Y-%m-%d")
        if max_invoice else ""
    )

    max_invoice_number = (
        max_invoice.so_hoa_don
        if max_invoice else ""
    )
    ...
    if request.method == "POST":
        ...
        if not ngay_hd_str:
            return HttpResponseBadRequest(
                "❌ Thiếu ngày lập phiếu"
            )

        try:
            ngay_hd = datetime.strptime(
                ngay_hd_str,
                "%Y-%m-%d"
            ).date()
        except ValueError:
        ...
        # 3. KIỂM TRA NGÀY
        # =========================
        force_continue = request.POST.get("force_continue") == "1"

        if max_invoice:
            ngay_hd_max = max_invoice.ngay_hd

            if ngay_hd > ngay_hd_max and not force_continue:

                return render(
                    request,
                    "create_export_invoice.html",
                    {
                        ...
                        "max_invoice_number": max_invoice_number,

                        "warning_date": ngay_hd.strftime("%d/%m/%Y"),
                        "warning_max_date": ngay_hd_max.strftime("%d/%m/%Y"),
                        "warning_invoice_number": max_invoice.so_hoa_don,
        ...
        try:
            items_count = int(
                request.POST.get("items_count", 0)
            )
        except ValueError:
            items_count = 0
