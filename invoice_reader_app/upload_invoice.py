...
from invoice_reader_app.model_invoice import Invoice, InvoiceItem, Supplier,Customer
from .forms import UploadFileForm
from django.views.decorators.csrf import csrf_protect
from django.contrib import messages
from .models_purchaseorder import BankPayment
...
from invoice_reader_app.utils.get_item_sku import get_item_sku, normalize_text
...
def save_invoice(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # ==========================================
            # TỰ ĐỘNG TẠO KHÁCH HÀNG CHO HÓA ĐƠN XUẤT
            # ==========================================
            if invoice.ma_so_thue == "0314858906":
                customer, created = Customer.objects.get_or_create(
                    ma_so_thue=invoice.ma_so_thue_mua,
                    defaults={
                        "ten_khach_hang": invoice.ten_nguoi_mua,
                        "dia_chi": invoice.dia_chi_mua,
                    }
                )
                ...
                if not created:
                    ...
                    if invoice.ten_nguoi_mua and customer.ten_khach_hang != invoice.ten_nguoi_mua:
                        customer.ten_khach_hang = invoice.ten_nguoi_mua
                        updated = True

                    if invoice.dia_chi_mua and customer.dia_chi != invoice.dia_chi_mua:
                        customer.dia_chi = invoice.dia_chi_mua
                        updated = True
    ...
    for inv in invoices:
        ...
        for item in items:
            product = ProductName.objects.filter(
                ten_hang__iexact=item.ten_hang
            ).first()

            item.sku_to_show = product.sku if product else ""
...
def invoice_list(request):
    ...
    for inv in invoices:
        ...
        for item in items:
            ...
            # Ưu tiên SKU đã lưu trong InvoiceItem
            if item.sku:

                item.sku_to_show = item.sku


            else:

                # fallback tìm trong danh mục hàng hóa
                product = ProductName.objects.filter(
                    ten_hang__iexact=item.ten_hang
                ).first()
        ...
        # giữ lại để template dùng
        inv.items_list = items
        # Tổng hợp số tiền từ XML
        inv.tong_tien_hang = sum(item.thanh_tien for item in items)
        inv.tong_chiet_khau = sum(item.chiet_khau for item in items)
        inv.tien_thue_gtgt = sum(item.tien_thue for item in items)
...
def delete_invoice(request, invoice_id):
    ...
    # Kiểm tra hóa đơn đã có Phiếu xuất chưa
    has_po = PurchaseOrder.objects.filter(invoice=invoice).exists()

    if has_po:
        messages.error(
            request,
            "Không thể xoá hóa đơn vì hóa đơn này đã có Phiếu xuất/chứng từ liên quan."
        )
        return redirect('invoice_list')

    # Chưa có chứng từ liên quan thì mới cho xoá
    invoice.delete()
...
def invoice_list(request):
    ...
    # Tạo dictionary SKU từ danh mục ProductName một lần duy nhất
    product_sku_map = {
        normalize_text(p.ten_hang): p.sku.strip()
        for p in ProductName.objects
            .exclude(sku__isnull=True)
            .exclude(sku="")
            if p.ten_hang and p.sku
    }
    ...
    for inv in invoices:
        ...
        for item in items:
            ...
            # -----------------------------------------
            # 1. Ưu tiên SKU đã lưu trong InvoiceItem
            # -----------------------------------------
            if item.sku:
                item.sku_to_show = item.sku.strip()

            # -----------------------------------------
            # 2. Nếu InvoiceItem chưa có SKU
            #    thì tìm trong ProductName
            # -----------------------------------------
            else:
                item.sku_to_show = product_sku_map.get(
                    normalize_text(item.ten_hang),
                    ""
                )
        ...
        # Giữ danh sách item đã được gán SKU
        inv.items_list = items

        # ================= TỔNG TIỀN =================

        inv.tong_tien_hang = sum(
            (item.thanh_tien or 0) for item in items
        )

        inv.tong_chiet_khau = sum(
            (item.chiet_khau or 0) for item in items
        )

        inv.tien_thue_gtgt = sum(
            (item.tien_thue or 0) for item in items
        )

        inv.tong_thanh_toan = sum(
            (item.thanh_toan or 0) for item in items
        )
    ...
    if search:
        ...
        for inv in data_list:
            ...
            haystack_fields = [
                remove_accents(inv.ten_dv_ban or ""),
                remove_accents(inv.ma_so_thue or ""),
                remove_accents(inv.so_hoa_don or ""),
            ]

            items_text = " ".join(
                remove_accents(item.ten_hang or "")
                for item in inv.items_list
            )

            haystack_fields.append(items_text)

            if any(
                search_norm in field.lower()
    ...
    for invoice in paginated_invoices:
        ...
        invoice.total_so_luong = (
            totals_items["total_so_luong"] or 0
        )

        invoice.total_thanh_tien = (
            totals_items["total_thanh_tien"] or 0
        )

        invoice.total_tien_thue = (
            totals_items["total_tien_thue"] or 0
        )
...
def parse_invoice_xml(xml_file):
    ...
    Nguyên tắc:
    - Không phụ thuộc namespace XML.
    - Không hard-code cấu trúc namespace.
    - Ưu tiên dữ liệu trực tiếp từ XML.
    - Không tự suy diễn SKU nếu XML chưa có.
    - SKU sẽ được xử lý chính thức sau khi tạo InvoiceItem
      thông qua get_item_sku().
    """
    ...
    def find_first(parent, *tag_names):
        """
        Tìm node đầu tiên theo tên tag,
        ...
        for elem in parent.iter():
            if local_name(elem.tag) in tag_names:
                return elem
    ...
    def get_text(parent, *tag_names):
        """
        Lấy text của node đầu tiên.
        """
        elem = find_first(parent, *tag_names)
    ...
    def get_all(parent, tag_name):
        """
        Lấy tất cả node cùng tên.
        """
        result = []
    ...
    if seller is not None:
        ...
        invoice["ten_ngan_hang"] = get_text(
            seller,
            "TenNganHang",
            "BankName",
            "Bank",
    ...
    # =========================================================
    # FALLBACK NGƯỜI BÁN
    # Một số XML không bọc trong NBan
    # =========================================================

    if not invoice["ten_dv_ban"]:
        invoice["ten_dv_ban"] = get_text(
            root,
            "Ten",
        )

    if not invoice["ma_so_thue"]:
        invoice["ma_so_thue"] = get_text(
            root,
            "MST",
        )

    if not invoice["dia_chi"]:
        invoice["dia_chi"] = get_text(
            root,
            "DChi",
        )
    ...
    if buyer is not None:
        ...
        invoice["ten_nguoi_mua"] = get_text(
            buyer,
            "Ten",
            "TenDonVi",
            "CompanyName",
        ...
        invoice["ma_so_thue_mua"] = get_text(
            buyer,
            "MST",
            "MaSoThue",
            "TaxCode",
    ...
    mst_ban = clean_text(
        invoice.get("ma_so_thue")
    )

    # Trường hợp hệ thống của bạn:
    # MST 0314858906 = hóa đơn xuất
    if mst_ban == "0314858906":
        invoice["loai_hd"] = "XUAT"
    else:
        invoice["loai_hd"] = "VAO"
    ...
    invoice["trang_thai"] = get_text(
        root,
        "TrangThai",
        "Status",
    )

    invoice["so_hoa_don_goc"] = get_text(
        root,
        "SHDonGoc",
        "SoHoaDonGoc",
        "SoHDGoc",
    ...
    invoice["ngay_hoa_don_goc"] = get_text(
        root,
        "NLapGoc",
        "NgayHoaDonGoc",
        "NgayHDGoc",
    ...
    invoice["ly_do_dieu_chinh"] = get_text(
        root,
        "LyDoDieuChinh",
        "LyDo",
    )
    ...
    item_elements = [
        elem
        for elem in root.iter()
        if local_name(elem.tag) == "HHDVu"
    ]

    for index, item_el in enumerate(
        item_elements,
        start=1
    ):
        ...
        # -----------------------------------------------------
        # SKU / MÃ HÀNG TRỰC TIẾP TRONG XML
        # -----------------------------------------------------
        ...
        thanh_tien_sau_ck = round(
            thanh_tien_truoc_ck - ck,
            2
        )
        ...
        thue_suat = parse_float(
            thue_suat_text
        )

        tien_thue_xml = parse_float(
            get_text(
                item_el,
                "TThue",
                "TienThue",
    ...
    invoice["tong_chiet_khau"] = round(
        tong_chiet_khau,
        2
    )

    invoice["tong_tien_hang"] = parse_float(
        get_text(
            root,
            "TgTCThue",
            "TongTienHang",
    ...
    invoice["tong_tien_thue"] = parse_float(
        get_text(
            root,
            "TgTThue",
            "TongTienThue",
    ...
    if not invoice["tong_tien_hang"]:
        invoice["tong_tien_hang"] = round(
            sum(
    ...
    if not invoice["tong_tien_thue"]:
        invoice["tong_tien_thue"] = round(
            sum(
                item["tien_thue"]
                for item in items
            ),
    ...
    if not invoice["tong_tien"]:
        invoice["tong_tien"] = round(
            sum(
                item["thanh_toan"]
                for item in items
            ),
