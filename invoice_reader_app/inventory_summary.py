...
from django.db.models import Sum, FloatField, Value, OuterRef, Subquery, CharField
...
def get_inventory_data(search=None):
    ...
    # Lấy tất cả các item phiếu nhập HH
    po_items_in_qs = PurchaseOrderItem.objects.filter(
        purchase_order__phan_loai_phieu='HH'
    )

    # Lấy tất cả các item phiếu xuất (nếu có)
    po_items_out_qs = PurchaseOrderItem.objects.filter(
        purchase_order__phan_loai_phieu='PX'
    )

    # Map nhập theo sku_auto
    grouped = {}
    for item in po_items_in_qs:
        ...
        ten_goi_chung = item.ten_goi_chung_auto
        dvt = item.unit
    ...
    for item in po_items_out_qs:
        ...
        if sku not in grouped:
            # Nếu SKU chỉ xuất mà chưa nhập, vẫn tạo record
            grouped[sku] = {
                'sku': sku,
                'ten_goi_chung': item.ten_goi_chung_auto,
                'dvt': item.unit,
