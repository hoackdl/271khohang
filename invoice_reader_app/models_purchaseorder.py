...
from .models_activitytype import ActivityType
...
class PurchaseOrder(models.Model):
    ...
    activity_type = models.ForeignKey(        ActivityType,        on_delete=models.PROTECT,   null=True,        blank=True,        related_name="purchase_orders",
...
class RevenueType(models.Model):
    code = models.CharField(
        max_length=50,
        unique=True,
        verbose_name="Mã loại hình"
    )

    name = models.CharField(
        max_length=255,
        verbose_name="Tên loại hình"
    )
...
class CostCenter(models.Model):
    activity_type = models.ForeignKey(        ActivityType,        on_delete=models.SET_NULL,        null=True,        blank=True,        related_name="cost_centers",        verbose_name="Loại hình hoạt động"    )
...
class BankPayment(models.Model):
    ...
    cost_centers = models.ManyToManyField(        "CostCenter",        blank=True,        related_name="bank_payments",        verbose_name="Trung tâm chi phí"    )
...
class BankPaymentAllocation(models.Model):
    ...
    logic_version = models.PositiveSmallIntegerField(        default=1    )
    created_at = models.DateTimeField(        auto_now_add=True    )
    revenue_type = models.ForeignKey(        RevenueType,        on_delete=models.SET_NULL,        null=True,        blank=True,        related_name="payment_allocations",
        verbose_name="Loại hình doanh thu"
    )
    activity_type = models.ForeignKey(    ActivityType,    on_delete=models.SET_NULL,    null=True,    blank=True,    related_name="payment_allocations",    verbose_name="Loại hình hoạt động")
    class Meta:
