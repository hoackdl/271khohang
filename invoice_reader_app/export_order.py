...
def export_order_list(request):
    ...
    if per_page.lower() != 'all':
        ...
        paginated_pos = pos  # Trường hợp 'all'
...
def create_or_update_purchase_order_from_invoice(invoice):
    """
    Tạo hoặc cập nhật PurchaseOrder cho invoice (phiếu xuất PX)
    """
    with transaction.atomic():  # đảm bảo toàn bộ là 1 transaction
        # --- Tạo/ lấy supplier ---
        supplier = invoice.supplier or Supplier.objects.get_or_create(
            ma_so_thue=invoice.ma_so_thue,
            defaults={
        ...
        # --- Tạo/ lấy customer (người mua) ---
        # --- Tạo/ lấy customer (người mua) ---
        if invoice.ma_so_thue_mua:
            # MST người mua có → dùng trực tiếp
            customer, created = Customer.objects.get_or_create(
                ma_so_thue=invoice.ma_so_thue_mua,
                defaults={
                    'ten_khach_hang': invoice.ten_nguoi_mua or 'Khách hàng',
            ...
            # MST trống → dùng placeholder
            placeholder_mst = f"TEMP-{invoice.id}"
            customer_name = invoice.ten_nguoi_mua or "Khách lẻ"
            customer, created = Customer.objects.get_or_create(
                ma_so_thue=placeholder_mst,
                defaults={
        ...
        if po:
            # Cập nhật tổng tiền và tax nếu PO đã tồn tại
        ...
        for item in invoice.items.all():
            ...
            # Lấy SKU ưu tiên từ invoice item, nếu không có → auto match
...
def create_or_update_purchase_order_from_invoice(invoice):
    """
    Tạo hoặc cập nhật PurchaseOrder từ Invoice (hóa đơn xuất PX)
    Luôn lấy customer từ invoice, đảm bảo tên khách hàng chính xác.
    """
    with transaction.atomic():
        ...
        if invoice.ma_so_thue_mua:
            ...
            customer, _ = Customer.objects.get_or_create(
                ma_so_thue=placeholder_mst,
                defaults={
                    'ten_khach_hang': invoice.ten_nguoi_mua or "Khách lẻ",
                    'dia_chi': invoice.dia_chi_mua or ""
                }
            )
...
def create_or_update_purchase_order_from_invoice(invoice: Invoice) -> PurchaseOrder:
    """
    Tạo hoặc cập nhật PurchaseOrder (PX) từ hóa đơn xuất.
    Quy tắc: 1 hóa đơn = 1 phiếu xuất
    """
    with transaction.atomic():
        # --- Supplier ---
        supplier = invoice.supplier
        ...
        if invoice.ma_so_thue_mua:
            ...
            # MST người mua trống → dùng placeholder
            placeholder_mst = f"TEMP-{invoice.id}"
            customer, _ = Customer.objects.get_or_create(
                ma_so_thue=placeholder_mst,
                defaults={
        ...
        # --- PO: check theo invoice để tránh trùng ---
        po_number = generate_px_number(invoice.ngay_hd, invoice.so_hoa_don)
