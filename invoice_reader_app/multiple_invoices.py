            ...
            for pack in invoices_list:
                ...
                invoice_obj, created = Invoice.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        "tong_tien": to_decimal(inv.get("tong_tien")),  # Chuyển sang Decimal
                        "file_name": file_name,
                    }
                )
                ...
                # Không gộp hàng, lưu mỗi item riêng biệt
                invoice_items = []
                for item in items:
                    ...
                    # Nếu thuế suất là dạng số (0.08 -> 8)
                    if isinstance(raw_tax, float) and raw_tax < 1:
                        raw_tax = raw_tax * 100

                    # Nếu thuế suất là dạng string có ký tự "%", loại bỏ dấu "%"
                    raw_tax = str(raw_tax).replace("%", "").strip()
                    tax_rate = to_decimal(raw_tax)

                    # Xử lý số lượng, đơn giá và tổng tiền
                    ...
                    invoice_items.append(
                        InvoiceItem(
                            ...
                            so_luong=quantity,
                            don_gia=unit_price,
                            thanh_tien=total_price,          # tiền trước thuế
                            chiet_khau=chiet_khau_item,      # tiền chiết khấu
                            thue_suat=tax_rate,
                            tien_thue=tien_thue,
                            thanh_toan=(total_price - chiet_khau_item + tien_thue).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP),
                ...
                # Lưu tất cả item vào cơ sở dữ liệu
                if invoice_items:
