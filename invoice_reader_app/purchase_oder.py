...
def edit_purchase_order(request, po_id):
    ...
    if request.method == 'POST':
        for item in items:
            ...
            so_luong_raw = request.POST.get(f'so_luong_{item.id}', item.quantity)
            so_luong_quy_doi_raw = request.POST.get(f'so_luong_quy_doi_{item.id}', item.so_luong_quy_doi or 0)
            don_gia_raw = request.POST.get(f'don_gia_{item.id}', item.unit_price)
            thue_suat_raw = request.POST.get(f'thue_suat_field_{item.id}', item.thue_suat_field or 0)

            # Chuyển về Decimal, bỏ dấu phẩy, an toàn với lỗi input
            try:
                quantity = Decimal(str(so_luong_raw).replace(',', ''))
            except (InvalidOperation, TypeError):
                quantity = item.quantity  # giữ nguyên nếu lỗi

            try:
                so_luong_quy_doi = Decimal(str(so_luong_quy_doi_raw).replace(',', ''))
            except (InvalidOperation, TypeError):
                so_luong_quy_doi = item.so_luong_quy_doi or 0

            try:
                unit_price = Decimal(str(don_gia_raw).replace(',', ''))
            except (InvalidOperation, TypeError):
                unit_price = item.unit_price

            try:
                thue_suat = Decimal(str(thue_suat_raw).replace(',', ''))
            except (InvalidOperation, TypeError):
                thue_suat = item.thue_suat_field or 0

            # Gán vào field database (không gán property)
            item.sku = sku
            item.ten_goi_chung = ten_goi_chung
            item.quantity = quantity
            item.so_luong_quy_doi = so_luong_quy_doi
            item.unit_price = unit_price
            item.thue_suat_field = thue_suat

            # Tính thành tiền & tiền thuế
            item.total_price = quantity * unit_price
            item.tien_thue_field = item.total_price * thue_suat / Decimal('100')
